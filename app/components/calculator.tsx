    ...
    try {
      ...
      for (let index = 0; index < formData.stages.length; index++) {
        ...
        if (stage.transportMethod === "") {
          ...
          setErrorMessage("Please choose a Transport Method for stage " + (index + 1));
          return;
        }

        if (stage.from == "" && stage.to == "" && stage.distance == "") {
          if (stage.transportMethod === "truck" || stage.transportMethod === "etruck") {
            setShowError(true);
            setShowMessage(false);
            setErrorMessage("Please specify either origin and destination address or distance for stage " + (index + 1));
            return;
          } else {
            setShowError(true);
            setShowMessage(false);
            setErrorMessage("Please specify a distance for stage " + (index + 1));
            return;
          }
        }

        const usesAddress = distanceOnly[index] && (stage.from !== "" || stage.to !== "");
        ...
        if (usesAddress) {
          if (!(stage.transportMethod === "truck" || stage.transportMethod === "etruck")) {
            setShowError(true);
            setShowMessage(false);
            setErrorMessage("Only `Truck` and `Electric Truck` allows for specifying origin and destination address for stage " + (index + 1));
            return;
          }
          ...
          if (stage.from == "" || stage.to == "") {
            setShowError(true);
            setShowMessage(false);
            setErrorMessage("Please specify both origin and destination address for stage " + (index + 1));
            return;
          }
          ...
          if (stage.distance) {
            setShowError(true);
            setShowMessage(false);
            setErrorMessage("Please specify either origin and destination address or distance, not both, for stage " + (index + 1));
            return;
          }
        ...
        // If we are using distances
          ...
          if (stage.distance < 1) {
            setShowError(true);
            setShowMessage(false);
            setErrorMessage("Distance must be greater than 0 for stage " + (index + 1));
            return;
          }
      ...
      setShowMessage(true);
      setShowError(false);
      setMessage(`Estimated CO2 emission for specified route: ${responseData.total_kg} kg`);
    } catch (error) {
        ...
        {[...Array(formData.stages.length)].map((_, index) => (
          <div className=" flex flex-col gap-4 ">
            ...
            <Label className="text-lg font-medium text-gray-900 dark:text-gray-100">
              {formData.stages.length <= 1 ? <>Transport Method:</> : <>Route Stage {index + 1}:</>}
            </Label>
            <Combobox
            ...
            {formData.stages.length <= 1 ? null :
              <Button onClick={handleRemoveStage(index)} className="w-full" variant={"destructive"} type="button">
                Remove Stage
              </Button>
            }
          </div>
